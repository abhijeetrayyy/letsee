...
| **065_unified_takes.sql** | Creates `takes` — one row per (user, thing, scope), one `body` plus an `is_public` flag instead of separate private/public columns. | ❌ **NOT APPLIED** | ✅ Yes (`if not exists`) | `/api/takes` degrades to an empty take rather than failing, so D1's pages render but nothing saves. |
| **066_related_by_audience.sql** | Adds `related_by_audience(item_id, item_type, limit)` — co-engagement counts over `user_title_affinity` for one title, for D5's "people here who watched this also watched" signal. | ❌ **NOT APPLIED** — written without database access, never executed, query plan never measured | ✅ Yes (`create or replace` in a transaction, creates no table) | D5's related section renormalises the community term away and ranks on keywords/director/collection instead. Privacy gates: public profiles only (NULL-safe predicate), `co_watchers >= 2`, and `seed_watchers >= 5` because `title_audience` names watchers on the same page. |
