...
**Algorithm**:
1. User selects: mood, genre, runtime, decade, media type.
2. Mood is mapped to TMDB genre IDs (e.g., "Thrilling" → Crime + Thriller + Mystery).
3. Filters are converted to TMDB discover params (`with_genres`, `primary_release_date.gte`, `with_runtime.gte`, etc.).
4. Results are shuffled (random) and filtered against user's watched/favorited items.
5. Top 10 picks are returned with posters, ratings, and metadata.
...
- Genre extraction: matches against known genre names ("thriller" → 53, "sci-fi" → 878)
- Year patterns: `2010s` → decade, `2020` → specific year, `before 2015` / `after 2000` → year range
- Actor extraction: "with Leonardo DiCaprio" → search TMDB person, extract ID
- "Similar to X": "similar to Inception" → search TMDB movie, use recommendations endpoint
- Media type detection: "tv" / "show" → TV, default → movie
- Fallback: If no structured params extracted, runs standard TMDB text search

**Examples**: "thriller from 2010s", "action with Keanu Reeves", "sci-fi after 2020", "horror similar to The Conjuring".
