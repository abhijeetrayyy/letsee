  ...
  return (
    <section className="relative w-full overflow-visible rounded-2xl isolate">
      ...
      <div className="relative px-4 sm:px-6 md:px-8 -mt-24 sm:-mt-28 md:-mt-32 pt-0">
        <div className="max-w-5xl flex flex-col sm:flex-row sm:items-end gap-6">
        ...
        {stats && (
          <div className="mt-6 grid grid-cols-4 sm:grid-cols-8 gap-3">
            <InlineStat label="Watched" value={stats.watchedCount} />
            <InlineStat label="Watching" value={stats.watchingCount} />
            <InlineStat label="Favorites" value={stats.favoriteCount} />
            <InlineStat label="Watchlist" value={stats.watchlistCount} />
          </div>
        )}
...
function InlineStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex flex-col items-center p-3 rounded-xl bg-surface-900/60 border border-surface-700/50">
      <span className="text-lg font-bold text-white">{value}</span>
      <span className="text-xs text-surface-400 uppercase tracking-wider mt-0.5">
        {label}
      </span>
    </div>
  );
