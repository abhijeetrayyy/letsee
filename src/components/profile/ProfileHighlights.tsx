...
export default function ProfileHighlights({
  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {featuredList && (
        <Link
          href={listPath(featuredList.id, featuredList.name)}
        ...
        >
          ...
          <span className="shrink-0 text-surface-500 group-hover:text-amber-400/80 text-sm font-medium transition-colors">
            View →
          </span>
        </Link>
      )}
      {pinnedReview && (
        ...
        >
          ...
          <span className="shrink-0 text-surface-500 group-hover:text-rose-400/80 text-sm font-medium transition-colors">
            View →
          </span>
        </Link>
      )}
