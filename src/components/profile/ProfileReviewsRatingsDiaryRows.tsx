...
export default function ProfileReviewsRatingsDiaryRows({
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-surface-700/60 bg-surface-900/40 p-6 flex flex-col items-center justify-center gap-3 min-h-[100px]">
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-surface-700/60 bg-surface-900/50 p-6 text-center">
  ...
  return (
    <div className="rounded-xl border border-surface-700/60 bg-surface-900/50 overflow-hidden">
      ...
      <div className="hidden sm:grid sm:grid-cols-12 gap-3 px-4 py-3 bg-surface-800/80 border-b border-surface-700/60 text-xs font-medium text-surface-400 uppercase tracking-wide">
      ...
      {items.map((row) => {
        ...
        return (
          <div
            ...
            className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3 px-4 py-4 border-b border-surface-700/40 last:border-b-0 hover:bg-surface-800/30 transition-colors"
          >
            <div className="sm:col-span-4 min-w-0">
              <Link
                href={href}
                className="text-white font-medium hover:text-brand-400 transition-colors line-clamp-2"
              >
                {row.item_name}
              </Link>
            ...
            <div className="sm:col-span-1 flex sm:justify-center items-start">
              <span className="sm:hidden text-xs text-surface-500 mr-2">Rating:</span>
              {row.score != null ? (
                <span className="inline-flex items-center px-2 py-1 rounded-md bg-accent-gold/20 text-accent-gold text-sm font-medium">
                  {row.score}/10
                </span>
              ) : (
            ...
            <div className={`min-w-0 ${isOwner ? "sm:col-span-4" : "sm:col-span-7"}`}>
              <span className="sm:hidden text-xs text-surface-500 block mb-1">Public review:</span>
              {reviewSnippet ? (
            ...
            {isOwner && (
              <div className="sm:col-span-3 min-w-0">
                <span className="sm:hidden text-xs text-surface-500 block mb-1">Your diary:</span>
                {diarySnippet ? (
                  <p className="text-sm text-surface-400 leading-relaxed line-clamp-3 italic">
                    {diarySnippet}
                  </p>
                ) : (
                  <span className="text-surface-600 text-sm">—</span>
